...
class HierarchicalDatasetBase(Dataset):
    ...
    def _song_id_to_indices(self):
        ...
        return np.concatenate([np.stack([np.ones(len(start_ids), dtype=np.int64) * song_id, start_ids], -1)
                               for song_id, start_ids in enumerate(self.start_ids_per_song)], 0)
...
class N2KeyDataset(HierarchicalDatasetBase):
    ...
    def __init__(self, analyses, shift_high=0, shift_low=0, max_l=256, max_h=12):
        super(N2KeyDataset, self).__init__(analyses, shift_high, shift_low, max_l, max_h)
        assert max_l >= 210
        self.key_rolls = [analysis['key_roll'] for analysis in analyses]
        self.phrase_rolls = [analysis['phrase_roll'][:, :, np.newaxis] for analysis in analyses]

        self.lengths = np.array([roll.shape[1] for roll in self.key_rolls])

        self.start_ids_per_song = [np.arange(0, 1, dtype=np.int64) for _ in range(len(self.lengths))]
...
class Key2RedDataset(HierarchicalDatasetBase):
    ...
    def __init__(self, analyses, shift_high=0, shift_low=0, max_l=128, max_h=128):
        super(Key2RedDataset, self).__init__(analyses, shift_high, shift_low, max_l, max_h)
        self.key_rolls = [analysis['key_roll'] for analysis in analyses]
    ...
    def get_data_sample(self, song_id, start_id, shift):
        # segment target data
        min_pitch, max_pitch = self.min_pitches[song_id] + shift, self.max_pitches[song_id] + shift
        if min_pitch < 48:
            shift = shift + 12 * int(np.ceil((48 - min_pitch) / 12))
        elif max_pitch > 108:
            shift = shift - 12 * int(np.ceil((max_pitch - 108) / 12))

        key_roll = self.key_rolls[song_id][:, start_id: start_id + self.max_l]  # (2, 128, 12)
        phrase_roll = self.phrase_rolls[song_id][:, start_id: start_id + self.max_l]  # (6, 128, 1)
        red_roll = self.reductions[song_id][:, start_id: start_id + self.max_l]  # (2, 128, 128)
        chord_roll = self.chord_rolls[song_id][:, start_id: start_id + self.max_l]  # (6, 128, 12)
        ...
        measure_importance = phrase_type_importance[measure_phrase_roll.argmax(0)][0: start_measure]
        ...
        for i in range(n_cond):
            ...
            acc_measure_importance = np.array([measure_importance[i: i + cond_scope].sum()
    ...
    def show(self, item, show_img=True):
        ...
        print(f"sample[{item}].shape = {sample.shape}), cond_sample[{item}].shape = {cond_sample.shape}")
        ...
        if show_img:
            ...
            for i in range(5):
                ...
                cond_img = np.pad(cond_img, pad_width=((0, 1), (0, 0), (0, 0)), mode='constant')
    ...
    def get_whole_song(self, song_id):
        ...
        chord_roll = np.roll(chord_roll, shift=shift, axis=-1)
        actual_l = red_roll.shape[1]
        target_l = max(int(np.ceil(red_roll.shape[1] / (self.max_l // 2)) * self.max_l // 2), self.max_l)
...
class Red2MelDataset(HierarchicalDatasetBase):
    def __init__(self, analyses, shift_high=0, shift_low=0, max_l=128, max_h=128):
        super(Red2MelDataset, self).__init__(analyses, shift_high, shift_low, max_l, max_h)
        self.key_rolls = [analysis['key_roll'] for analysis in analyses]
        self.expand_key_rolls()
    ...
    def expand_rough_chord_rolls(self):
        self.rough_chord_rolls = [expand_roll(roll, nspb, contain_onset=True)
                                  for roll, nspb in zip(self.rough_chord_rolls, self.nspbs)]

    def expand_chord_rolls(self):
        self.chord_rolls = [expand_roll(roll, nspb, contain_onset=True)
                            for roll, nspb in zip(self.chord_rolls, self.nspbs)]

    def expand_reductions(self):
        self.reductions = [expand_roll(roll, nspb, contain_onset=True)
    ...
    def get_data_sample(self, song_id, start_id, shift):
        ...
        phrase_roll = self.phrase_rolls[song_id][:, start_id: start_id + self.max_l]  # (6, 128, 1)
        red_roll = self.reductions[song_id][:, start_id: start_id + self.max_l]  # (2, 128, 128)
        rough_chord_roll = self.rough_chord_rolls[song_id][:, start_id: start_id + self.max_l]  # (6, 128, 12)
    ...
    def get_cond_img(self, song_id, start_id, end_id, shift):
        ...
        phrase_roll = self.phrase_rolls[song_id][:, start_id: end_id]  # (6, 128, 1)
        red_roll = self.reductions[song_id][:, start_id: end_id]  # (2, 128, 128)
        rough_chord_roll = self.rough_chord_rolls[song_id][:, start_id: end_id]  # (6, 128, 12)
    ...
    def get_whole_song(self, song_id):
        ...
        chord_roll = np.roll(chord_roll, shift=shift, axis=-1)
        actual_l = mel_roll.shape[1]
        target_l = max(int(np.ceil(mel_roll.shape[1] / (self.max_l // 2)) * self.max_l // 2), self.max_l)
...
class Mel2AccDataset(HierarchicalDatasetBase):
    def __init__(self, analyses, shift_high=0, shift_low=0, max_l=128, max_h=128):
        super(Mel2AccDataset, self).__init__(analyses, shift_high, shift_low, max_l, max_h)
        self.key_rolls = [analysis['key_roll'] for analysis in analyses]
        self.expand_key_rolls()
    ...
    def get_data_sample(self, song_id, start_id, shift):
        ...
        for slc in slices:
            seg_start_id, seg_end_id = slc[0] * nbpm * nspb, slc[1] * nbpm * nspb
            seg_lgth = seg_end_id - seg_start_id
            cond_seg = self.get_cond_img(song_id, seg_start_id, seg_end_id, shift, acc_shift)
            cond_img[:, cur_step: cur_step + seg_lgth] = cond_seg
            cur_step += seg_lgth + interval
...
def create_train_and_valid_analyses(truncate_length=None):
    ...
    for i in tqdm(range(909), desc='loading dataset'):
        ...
        song = read_data(
            ...
            num_step_per_beat=4, clean_chord_unit=num_beat_per_measure,
            song_name=folder_name, label=label)
...
def create_n2key_datatsets(train_analyses, valid_analyses, shift_high_t=5, shift_low_t=-6,
                           shift_high_v=0, shift_low_v=0, max_l=256, max_h=12, truncate_length=None):
    train_dataset = N2KeyDataset(train_analyses, shift_high_t, shift_low_t, max_l, max_h)
    valid_dataset = N2KeyDataset(valid_analyses, shift_high_v, shift_low_v, max_l, max_h)
    return train_dataset, valid_dataset


def create_key2red_datasets(train_analyses, valid_analyses, shift_high_t=5, shift_low_t=-6,
                           shift_high_v=0, shift_low_v=0, max_l=128, max_h=128, truncate_length=None):
    train_dataset = Key2RedDataset(train_analyses, shift_high_t, shift_low_t, max_l, max_h)
    valid_dataset = Key2RedDataset(valid_analyses, shift_high_v, shift_low_v, max_l, max_h)
    return train_dataset, valid_dataset


def create_red2mel_datasets(train_analyses, valid_analyses, shift_high_t=5, shift_low_t=-6,
                            shift_high_v=0, shift_low_v=0, max_l=128, max_h=128, truncate_length=None):

    train_dataset = Red2MelDataset(train_analyses, shift_high_t, shift_low_t, max_l, max_h)
    valid_dataset = Red2MelDataset(valid_analyses, shift_high_v, shift_low_v, max_l, max_h)
    return train_dataset, valid_dataset


def create_mel2acc_datasets(train_analyses, valid_analyses, shift_high_t=5, shift_low_t=-6,
                            shift_high_v=0, shift_low_v=0, max_l=128, max_h=128, truncate_length=None):

    train_dataset = Mel2AccDataset(train_analyses, shift_high_t, shift_low_t, max_l, max_h)
    valid_dataset = Mel2AccDataset(valid_analyses, shift_high_v, shift_low_v, max_l, max_h)
    return train_dataset, valid_dataset
...
def compute_min_max_pitch():
    ...
    for i in tqdm(range(909), desc='checking min/max pitches'):
        ...
        song = read_data(
            data_fn, num_beat_per_measure=num_beat_per_measure,
            num_step_per_beat=4, clean_chord_unit=num_beat_per_measure,
            song_name=folder_name, label=label)
