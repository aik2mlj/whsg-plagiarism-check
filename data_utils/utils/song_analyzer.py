...
class Pop909Song:
    ...
    def compute_total_measure(self):
        ...
        if self.acc is None:
            ...
            last_step = max(
                (self.melody[:, 0] + self.melody[:, 2]).max(),
    ...
    def regularize_phrases(self):
        ...
        self.phrase_lengths = np.append(self.phrase_lengths, extra_phrase_length)
        self.phrase_starts = np.append(self.phrase_starts, original_phrase_length)
    ...
    def create_a_phrase_level_dict(self, phrase_id):
        ...
        phrase_dict = {
            'phrase_name':
                self.phrase_names[phrase_id],
            'phrase_type':
                self.phrase_types[phrase_id],
            'phrase_length':
                self.phrase_lengths[phrase_id],
            'start_measure':
                start_measure,
            'end_measure':
                end_measure,
            ...
            'measures':
                [
                    self.create_a_measure_level_dict(measure_no)
    ...
    def create_song_level_dict(self):
        song_dict = {
            ...
            'phrases':
                [
                    self.create_a_phrase_level_dict(phrase_id)
    ...
    def get_rough_chord(self):
        ...
        while i < len(self.chord):
            c_i = self.chord[i]
            onset_i, root_i, chroma_i, bass_i, duration_i = c_i[0], c_i[1], c_i[
                2 : 14], c_i[14], c_i[15]
            ...
            while j < len(self.chord) and duration < self.clean_chord_unit:
                c_j = self.chord[j]
                onset_j, root_j, chroma_j, bass_j, duration_j = c_j[0], c_j[1], c_j[
                    2 : 14], c_j[14], c_j[15]
            ...
            rough_chord.append(
                np.concatenate(
                    [np.array([onset_i, root]), chroma,
                     np.array([bass, duration])]
                )
            )
            i = j
    ...
    def get_pitch_rhythm_contours(self):
        ...
        rough_p_contour = extract_pitch_contour(
            pr_contour, self.num_step_per_beat, stride=self.num_beat_per_measure / 2.
        )
        detailed_p_contour = extract_pitch_contour(
            pr_contour, self.num_beat_per_measure, stride=1.
        )

        rhythm = pr_array_to_rhythm(pr_array)
        rough_r_contour = extract_rhythm_intensity(
            rhythm,
            self.num_step_per_beat,
            stride=self.num_beat_per_measure / 2.,
            quantization_bin=4
        )
        detailed_r_contour = extract_rhythm_intensity(
            rhythm, self.num_step_per_beat, stride=1., quantization_bin=3
        )
    ...
    def analyze(self):
        ...
        key_roll = get_key_roll(
            pr_mel, pr_chord, self.phrase_starts, self.phrase_lengths,
            self.total_measure, self.num_beat_per_measure, self.num_step_per_beat
        )

        # rough_p_contour, detailed_p_contour, rough_r_contour, detailed_r_contour = self.get_pitch_rhythm_contours()

        phrase_roll = phrase_to_phrase_roll(
            self.phrase_starts, self.phrase_lengths, self.phrase_types,
            self.total_measure
        )
        ...
        analysis = {
            ...
            "key_roll": key_roll,
            # beat-level
            "mel_reduction": reduction_roll,
            "chord_roll": pr_chord,
            "rough_chord_roll": pr_rough_chord,
            # step-level
            "mel_roll": pr_mel,
            # # rough style
