...
# def edit_path(path_density, max_path_density, path_bins, path, note_mat,
#               chord_mat):
#     edit_record = [[] for _ in range(len(path_density))]
#     modified_path_density = path_density.copy()
#     n_chord = len(path_density)
#
#     # prev_cd = None
#     for i in range(n_chord - 1, -1, -1):
#         density, max_density = path_density[i], max_path_density[i]
#         path_bin = path_bins[i]
#
#         while density > max_density:
#             # check prolongation
#             for j, path_idx in enumerate(path_bin):
#                 note_id = path[path_idx]
#                 if path_idx == 0:
#                     continue
#                 else:
#                     prev_note_id = path[path_idx - 1]
#
#                     if note_mat[note_id, 2] == note_mat[prev_note_id, 2]:
#                         edit_record[i].append((j, 'r'))
#                         density -= 1
#                     if density <= max_density:
#                         break
#             else:
#                 for j in range(len(path_bin) - 1, -1, -1):
#                     if i == n_chord - 1:
#                         break
#
#                     if j in [m[0] for m in edit_record[i]]:
#                         continue
#
#                     path_idx = path_bin[j]
#                     note_id = path[path_idx]
#                     note = note_mat[note_id]
#
#                     # check note is chord tone of chord[i + 1]
#                     if chord_mat[i + 1, int(note[2] % 12) + 1] == 1 and \
#                         modified_path_density[i + 1] < max_path_density[i + 1]:
#                         edit_record[i].append((j, 'm'))
#                         density -= 1
#                         modified_path_density[i + 1] += 1
#                         break
#                 else:
#                     for j in range(len(path_bin) - 1, -1, -1):
#                         if j not in [m[0] for m in edit_record[i]]:
#                             edit_record[i].append((j, 'd'))
#                             density -= 1
#                         if density <= max_density:
#                             break
...
#         chord_id = int(note[6]) \
#             if int(note[6]) < len(chord_density) else int(note[5])
#         if not 0 <= chord_id < len(chord_density):
#             print(chords)
#             print(note)
#         chord_density[chord_id] += 1
#         chord_bin[chord_id].append(pid)
#     # print(chord_density, chord_max_density, 'x')
#     return chord_density, chord_max_density, chord_bin


def check_path(path, note_mat, chord):
    if path is None:
        return None, None
...
def assign_duration(chord_mat, path_bin, nspb):
    if path_bin is None:
